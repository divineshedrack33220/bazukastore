...
exports.updateProfile = async (req, res) => {
  try {
    const { name, phone, shopName, avatar } = req.body;  // Extended: Add shopName/avatar for stores
    const user = await User.findById(req.user._id);
    if (!user) return res.status(404).json({ message: 'User not found' });

    if (name) user.name = name;
    if (phone) user.phone = phone;
    if (shopName) user.shopName = shopName;  // New: Update shop name for seller profiles
    if (avatar) user.avatar = avatar;  // New: Update avatar URL (e.g., from upload endpoint)
