...
exports.updateOrderStatus = async (req, res) => {
  try {
    ...
    // --- OFFLINE PUSH: Send to user if offline ---
    const title = `Order #${order.orderNumber} Updated`;
    const body = `Status: ${status}`;
    const url = `/orders.html`;

    await sendPushToUser(order.user._id, title, body, url);
    console.log(`[PUSH] Order status update sent to user ${order.user._id}`);
...
exports.verifyPaymentProof = async (req, res) => {
  try {
    ...
    if (paymentStatus === 'completed') {
      ...
      // --- OFFLINE PUSH: Payment verified ---
      const title = `Payment Verified!`;
      const body = `Order #${order.orderNumber} is confirmed.`;
      const url = `/orders.html`;

      await sendPushToUser(order.user._id, title, body, url);
      console.log(`[PUSH] Payment verified push sent to user ${order.user._id}`);
    }
